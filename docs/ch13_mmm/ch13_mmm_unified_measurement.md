...
The walkthrough and exercise-solutions notebooks (`ch13_walkthrough.ipynb`, `ch13_exercise_solutions.ipynb`) run every listing above, and every artifact behind it, against the same seed. The resource allocation chapter turns the unified budget recommendation produced here into a territory-level call plan under rep capacity constraints.
